"""
Test Manager
Handles test flow, timer, and state management
"""
...
from models import TestStep, TestStatus, InputType, TimerStatus
...
class TestManager(QObject):
    ...
    def _start_current_step(self):
        ...
        logger.info(f"Started step {self.current_step_index + 1}/{len(self.steps)}: {current_step.name}")
    ...
    def submit_result(self, result_value: Any) -> bool:
        ...
        if current_step.input_type == InputType.PASS_FAIL:
            current_step.status = TestStatus.PASSED if result_value == "PASS" else TestStatus.FAILED
    ...
    def get_current_step(self) -> Optional[TestStep]:
        ...
        if self._is_valid_step_index(self.current_step_index):
            return self.steps[self.current_step_index]
        return None
    
    def get_step(self, index: int) -> Optional[TestStep]:
        """Get a specific step by index"""
        if self._is_valid_step_index(index):
    ...
    def get_progress_percentage(self) -> float:
        ...
        completed = sum(1 for step in self.steps if step.status in [TestStatus.PASSED, TestStatus.FAILED])
